...
exports.protect = catchAsync( async (req, res, next ) => {
    ...
    if(!token) {
        return next(
            new AppErrorHandling('You are not logged in! Please login to get access.', 401)
        ); 
    }
...
exports.restrictTo = (...roles) => {
    return (req, res, next) => {
        ...
        if(!roles.includes(req.user.role)) {
            return next(new AppErrorHandling('You do not have permission to perform this action', 403));
        }
...
exports.forgotPassword = catchAsync( async (req, res, next) => {
    ...
    if(!user) {
        return next(new AppErrorHandling('There is no user with email address.', 404));
    }
    ...
    try {
        ...
        return next(new AppErrorHandling('There was an error sending the email. Try again later!', 500));
    }
});
